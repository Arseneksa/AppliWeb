...
#    
    ...
    #return HttpResponse( )
#    
    offres_proche = Job.objects.filter(localisation = request.session['ville']).order_by('-pubdate')[0:6]
    offres_recente = Job.objects.all().order_by('-pubdate')[0:6]
    
    return render(request,'index.html', locals(), {'offres_proches':offres_proche, 'offres_recentes':offres_recente})
...
def envoyer_mail(request):
    ...
    if request.method == "POST":
            ...
            if subject and message and from_email:         
                try:             
                    send_mail(subject, message, from_email, ['soumeua@gmail.com'], fail_silently=False)         
                except BadHeaderError:  
                    return render(request,'about.html',{'form': form, 'error_message':'Mauvaise entête.'})        
                return render(request,'about.html',{'form': form, 'message':'Email envoyé avec succès'})   
            else:         

                return render(request,'about.html',{'form': form, 'error_message':'Verifiez que tous les champs ont été rempli et sont valide.'})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if User.objects.filter(email=form.cleaned_data['email']).exists():
                ...
                return render(request, template, {
                    'form': form, 
                    'error_message': 'Le nom existe déja.'
                })
...
def home(request):
    ...
    offres = Job.objects.filter(
           ...
           title__icontains = user.specialite
    ...
    #resupération de liste des favoris de l'utilisateur  
    #pour désactiver le boutton favoris des offres déjà ajouté aux favoris
    favoris = Favoris.objects.filter(user__id=user_id, visible = 1)
    liste_favoris = list(favoris)
    liste_job_fav = []
    for fav in liste_favoris:
        liste_job_fav.append(fav.job_id)
    ...
    #return HttpResponse(liste_job_fav)
    
    return render(request,'account/home.html', {'offres':offres, 'pages':pages, 'liste_job_fav':liste_job_fav})
...
def addfavoris(request, job_id): 
    ...
    favoris = Favoris( user= user , job= job, visible = 1)
    
    if not Favoris.objects.filter(user=user, job=job, visible=1).exists():
        favoris.save()
    ''' if Favoris.objects.filter(user=user, job=job, visible = 0).exists():
        favoris.visible = 1 
        favoris.save() '''
    #return HttpResponse(favoris.visible)
    #offres = Job.objects.filter(town__icontains = 'douala')
    
    return redirect('accounthome')


def deletefavoris(request, job_id):
    user_id = request.user.id

    user = Utilisateur.objects.get(id=user_id)

    job = Job.objects.get(id=job_id)
    favoris = Favoris.objects.get(user_id=request.user.id, job_id = job_id, visible = 1)
    favoris.visible = 0
    favoris.save()
